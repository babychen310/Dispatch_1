...
- **Cost coefficients**: Based on the power generation cost levels published by Zhongyuan Tianhong, sets of cost coefficients are randomly sampled from normal distributions to simulate real-world cost fluctuations. The sampling is post-processed to guarantee all values are positive and to enforce the ordering **storage discharge cost < wind power cost < renewable energy cost < coal-fired power cost < storage charging cost**. The generated coefficients are saved to `energy_costs.xlsx` and used as inputs to the dispatch optimization.
- Renewable penetration constraint: ≥ 50% of total generation.
- Wasserstein DRO robustness radius: ε = 0.05.
- **Solver**: The dispatch optimization is modeled and solved using **PuLP** (Python Linear Programming). The default PuLP solver backend is CBC (Coin-or Branch and Cut).
